  ...
  #pragma omp parallel private(i,j,k) 
  {
    #pragma omp sections  
    {
      ...
      { 
        ...
        /* OPCIÓN B: Un solo bucle para recorrer i y j*/ 
        #pragma omp parallel for private(i) schedule(static, chunk)
        for(i = 0; i < TAM*TAM; i++){
          mat_suma[i/TAM][i%TAM] = (A[i/TAM][i%TAM]+B[i/TAM][i%TAM]);  
      ...
      {
        /* OPCIÓN B: Un solo bucle para recorrer i y j*/ 
        #pragma omp parallel for  private(i, k) schedule(static, chunk)
        for(i = 0; i < TAM*TAM; i++){
          aux = 0;
          for(k=0;k<TAM;k++){
            aux+=(A[i/TAM][k]*B[k][i%TAM]);
          }
          mat_mult[i/TAM][i%TAM] = aux; 
        } // end omp parallel for
        /* FIN OPCIÓN B */
